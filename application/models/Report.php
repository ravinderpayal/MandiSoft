...
class Report extends CI_Model {
    ...
    function get_all_sales() {
        $query = $query = $this->db->query('SELECT item_lot,quantity,rate,week(sale_date) as sweek FROM `item_sale`,`accounts` where acc_id = sold_to and 1 = quantity_type');
        return $query->result();
    }
    ...
    function daywiseSaleAmount($l = 2) {
        $query = $query = $this->db->query('SELECT sum(f.rate * f.quantity) as amount,f.sale_date FROM (
         SELECT * FROM `item_sale` WHERE sale_date>CURRENT_DATE - INTERVAL '.$l.' MONTH
         ) as f GROUP BY WEEK(f.sale_date)');
        return $query->result();
    }
    function daywisePaymentAmount($l = 2) {
        $query = $query = $this->db->query(
        'SELECT * FROM (
        SELECT make_date,sum(amount) as Camount FROM `payment` GROUP BY WEEK(make_date)  ORDER BY make_date DESC LIMIT '.$l.'
        ) as f ORDER BY f.make_date ASC');
        $query = $query = $this->db->query('SELECT sum(f.amount) as Camount,f.make_date FROM (
         SELECT * FROM `payment` WHERE make_date>CURRENT_DATE - INTERVAL '.$l.' MONTH
         ) as f GROUP BY WEEK(f.make_date)');
        return $query->result();
    }
    function daywiseAccountBalance($l = 2) {
        $query = $query = $this->db->query('SELECT sum(f.amount) as Camount,f.make_date FROM (
         SELECT * FROM `payment` WHERE make_date>CURRENT_DATE - INTERVAL '.$l.' MONTH
         ) as f GROUP BY WEEK(f.make_date)');
        return $query->result();
    ...
    function bigDebts($l=20){
        $query = $this->db->query(
                'SELECT  acc.`acc_id`,acc.`ledger_number`,`acc_name`,acc.`account_balance`,acc.`datetime`,
                (select sum(sale.`rate` * sale.`quantity` - sale.`rebate`) from `item_sale` as sale where `sold_to` = acc.`acc_id`) as total_sale 
                FROM `accounts` as acc WHERE acc.`group_id`=1 ORDER BY acc.`account_balance` ASC  LIMIT '.$l
                );
        if($query)return $query->result();
    }
    /*
