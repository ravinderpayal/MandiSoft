...
Class account extends CI_Model {
    ...
    function search($s){
        $query = $this->db->like("acc_name",$s,"both",true)->or_like("acc_name_ll",$s,"both",false)->or_like("acc_area",$s,"both",true)->or_like("acc_city",$s,"both",true)->or_like("mobile_number1",$s,"both",true)->or_like("mobile_number2",$s,"both",true)->get('accounts');
        return $query->result();
    }
    ...
    function is_account($a){
        $query=$this->db->get_where("accounts",array("acc_id"=> $a));
                return ($query->num_rows()==1);
    }
    ...
    function get_ledger_sales($a,$b=100,$c=100,$d,$e){
                $query = $query = $this->db->query(
                        ...
                        . " s.rebate AS sale_rebate,s.item_lot,s.datetime,s.sale_date AS sale_date, dab.old_balance, dab.new_balance FROM `item_sale` as s"
    ...
    function get_ledger_payment($a,$b=20,$c=20){
                $query = $query = $this->db->query(
                        "SELECT * FROM (SELECT  p.amount as amount_received,p.rebate,date(p.`datetime`) as payment_date,p.`make_date`, dab.old_balance, dab.new_balance FROM `payment` as p LEFT JOIN `datewise_account_balance` dab ON (dab.payment_id = p.payment_id)  JOIN accounts as a ON(p.acc_id = a.acc_id) WHERE a.ledger_number=$a ORDER BY p.`make_date` DESC LIMIT $b ) as f ORDER BY f.make_date ASC LIMIT $c"
                );
                return $query->result();
    }
