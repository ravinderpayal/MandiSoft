...
if(account_balance<0,CONCAT("₹",(-1*account_balance)," Dr"),CONCAT("₹",account_balance," Cr")) as "Closing  Balance" FROM `accounts`
                        ...
                        foreach ($payments as $pay){
                            echo "<b>₹".$pay->amount_received.($pay->rebate?(" + ₹$pay->rebate REBATE( छुट )"):(""))."</b>&nbsp;&nbsp;&nbsp;&nbsp;<span style=\"float:right; display:inline-block\">".date("d/m/Y",strtotime($pay->make_date)?strtotime($pay->make_date):strtotime($pay->payment_date))."</span>"
                                    ."<br/>"
