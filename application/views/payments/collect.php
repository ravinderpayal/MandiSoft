...
<div class="container" style="margin-top:51px;">
    ...
    <table id="collect_payment" border="1px">
        <tr>
            <td>
                <label>Account Name<br/><input id="acc_name_visible" autofocus="true" /></label>
                <label>Ledger Number<br /><input type="number" id="acc_ledger_number" name="ledger_number" /></label>
                <label>Account Balance.<input disabled="" id="account_balance" style="width:300px"  /></label><input type="hidden"id="new_sale_account_id"  name="acc_num" value="<?php echo set_value("acc_num"); ?>" />
...
<script>
    ...
    $.ajax({url: base_url + 'api/customers', success: function (result) {
            ...
            $("#acc_name_visible").autocomplete({
                  ...
                  select: function( event, ui ) {
                    ...
                    $( "#account_balance" ).val( ui.item.ledger_balance);
    ...
    function updateByLedgerNum(a){
        if(ledger_array[a]){
            setAccount(ledger_array[a].ledger_balance,a,ledger_array[a].name)
        }
        else{
            acc404(2);
        }
